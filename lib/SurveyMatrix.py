...
class Survey_matrix(MatrixDictionary):
    ...
    def from_file(self, file, trans = False):
        ...
        rows    = []
        counts  = []
        found = False # Search for line starting with #OTU ID
        pattern = re.compile('#OTU ID|OTU_id')
        last_col = -1
        for line in f:
            if found:
                ...
                try:
                    counts.append( map(lambda x: float(x), fields[1:last_col]) )
                except:
                    print line
                    print fields
            else:
                if pattern.match(line.strip()):
                    ...
                    sys.stderr.write("Matched header: " + str(tmp) + "\n")
                    if(tmp[len(tmp)-1] == "taxonomy"):
                        last_col = -1
                    else:
                        last_col = len(tmp) - 1

                    cols = tmp[1:last_col]
                    sys.stderr.write("cols: " + str(cols) +  "\n")
                    found = True
        
        sys.stderr.write("Number of OTUs: " + str(len(rows)) + "\n")
        sys.stderr.write("Number of samples: " + str(len(cols)) + "\n")
        f.close()
